...
def M_laplaz(Nf,Nc):# No se si son estrictamente cuadradas
    ...
    matriz=np.eye(Nf,Nc,dtype=np.single)*2
    matriz_up=np.eye(Nf,Nc,k=1,dtype=np.single)*-1
    matriz_down=np.eye(Nf,Nc,k=-1,dtype=np.single)*-1
...
ax[0].set_title('Rendimiento inversa caso 2 single')
ax[0].set_xscale('log')
ax[0].set_yscale('log')
...
lista_B=[]
for i in lista_N: 
    lista_B.append(2*(i**2)*4)#1 single = 4 bytes
...
plt.tight_layout()
plt.savefig("scipy_false_single.png")

...
def M_laplaz(Nf,Nc):# No se si son estrictamente cuadradas
    ...
    matriz=np.eye(Nf,Nc,dtype=np.double)*2
    matriz_up=np.eye(Nf,Nc,k=1,dtype=np.double)*-1
    matriz_down=np.eye(Nf,Nc,k=-1,dtype=np.double)*-1
...
ax[0].set_title('Rendimiento inversa caso 2 double')
ax[0].set_xscale('log')
ax[0].set_yscale('log')
...
lista_B=[]
for i in lista_N: 
    lista_B.append(2*(i**2)*8)#1 double = 8 bytes
#se ocupan 2 matrices para invertir esto lo explico en el "README.md" del "github".
...
plt.tight_layout()
plt.savefig("scipy_false_double.png")

...
def M_laplaz(Nf,Nc):# No se si son estrictamente cuadradas
    ...
    matriz_up=np.eye(Nf,Nc,k=1,dtype=np.half)*-1
    matriz_down=np.eye(Nf,Nc,k=-1,dtype=np.half)*-1
...
ax[0].set_title('Rendimiento inversa caso 2 half')
ax[0].set_xscale('log')
ax[0].set_yscale('log')
...
lista_B=[]
for i in lista_N: 
    lista_B.append(2*(i**2)*2)#1 half = 2 bytes
...
plt.tight_layout()
plt.savefig("scipy_false_half.png")
